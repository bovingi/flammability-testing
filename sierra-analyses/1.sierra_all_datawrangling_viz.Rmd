...
source(here::here("sierra-analyses", "scripts_functions", "plot_facet_flam.R"))
source(here::here("sierra-analyses", "scripts_functions", "figure_info_sierra_flammability.R"))
```
...
```{r}
flam_seki_raw <- read_csv(here('sierra-analyses', "processed-data", "seki_flam_data_all.csv")) %>% 
  clean_names() %>% 
  filter(model == "HP") %>% 
...
flam_czo_2020_raw <- read_csv(here('sierra-analyses', "processed-data", "czo_2020_flam_data_all.csv")) %>% 
  clean_names() %>% 
  filter(model == "HP") %>% 
  mutate(location = "czo") %>% 
...
flam_czo_2021_raw <- read_csv(here('sierra-analyses', "processed-data", "czo_2021_flam_data_all.csv")) %>% 
  clean_names() %>% 
  filter(model == "HP") %>% 
  mutate(location = "czo", 
...
flam_sierra_df_2 %>% 
  write_csv(here('sierra-analyses', "processed-data", "sierra_flam_data_all.csv"))
```
