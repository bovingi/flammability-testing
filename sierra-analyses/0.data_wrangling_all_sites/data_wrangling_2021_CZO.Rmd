...
source(here::here("sierra-analyses", "scripts_functions", "outlierKD2.R"))
```
...
```{r}
raw_data_czo <- read_excel(here('sierra-analyses', "raw-data", "r.flam.data.czo.2021.xlsx"), sheet = "all", skip = 0, na = "NA") %>% 
  clean_names() %>%
  filter(lfm > 0) %>%
...
write_csv(rwc_data, here('sierra-analyses', "processed-data", "czo_2021_flam_curve_physiological_data.csv"))
...
write_csv(x = hp_ignite_only_pca, (here('sierra-analyses', "processed-data", "czo_2021_flam_data_all.csv")))
```
