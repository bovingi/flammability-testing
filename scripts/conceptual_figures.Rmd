...
r.flamm.index <- conceptual_flam_remef %>% 
  ...
  mutate(ignitability = (prop_ig+0.013746)*2.182502, combustibility = (((fh + heat_flux_change + temp_change)/3)+0.934473)*0.35772, sustainability = (fd+0.855757)*0.962559) %>% 
  dplyr::select(species, ignitability, combustibility, sustainability) %>% 
  ungroup() %>% 
...
r.flamm.index.top <- interspecific_flam_remef %>% 
  drop_na(r_fh, r_fd, prop_ig, r_temp_change, r_heat_flux_change) %>% 
  group_by(species) %>% 
  summarise(fh = mean(r_fh), fd = mean(r_fd), prop_ig = mean(prop_ig),
            temp_change = mean(r_temp_change), heat_flux_change = mean(r_heat_flux_change)) %>% 
...
title(r.flamm.index.top[i,1], cex.main = 1.75)

# Add an arrow
TernaryArrows(c(50, 50, 50), c(r.flamm.index.top$ignitability[i], 0.03, 0.03), length = 0.2*r.flamm.index.top$ignitability[i], lwd = 5*r.flamm.index.top$ignitability[i], col = "black")
TernaryArrows(c(50, 50, 50), c(0.03, r.flamm.index.top$combustibility[i], 0.03), length = 0.2*r.flamm.index.top$combustibility[i], lwd = 5*r.flamm.index.top$combustibility[i], col = "black")
TernaryArrows(c(50, 50, 50), c(0.03, 0.03, r.flamm.index.top$sustainability[i]), length = 0.2*r.flamm.index.top$sustainability[i], lwd = 5*r.flamm.index.top$sustainability[i], col = "black")
}
flam.triangles <- recordPlot()
```
...
flam_scores <- data.frame(
  ...
  flam.score = r.flamm.index2$flam.score,
...
```{r}
# flam scores
r.flamm.index2.top <- interspecific_flam_remef %>% 
  drop_na(r_fh, r_fd, prop_ig, r_temp_change, r_heat_flux_change) %>% 
  group_by(species) %>% 
  summarise(fh = mean(r_fh), fd = mean(r_fd), prop_ig = mean(prop_ig),
            temp_change = mean(r_temp_change), heat_flux_change = mean(r_heat_flux_change)) %>% 
...
flam_scores <- data.frame(
  species = flam_EScores_subscores$Species,
  flam.score = r.flamm.index2.top$flam.score,
  es.score = flam_EScores_subscores$ES.score
) %>% 
  mutate(flam.score = scale(flam.score),
         es.score = scale(es.score))

# Visualization
# 'ARTCAL', 'ARCDEN', 'CEAGRI', 'ERIKAR', 'HETARB', 'MALLAU', 'SALAPI', 'SALLEU'
flam_scores$labx <- c(0.37, -0.35, -1.06, -1.92, 0.31, 0.38, 0.65, 0.55)
flam_scores$laby <- c(-0.42, -0.35, 1.34, -1.22, 0.1, -0.6, -0.1, 1.18)
...
```{r}
plot_grid(es.vs.flam, flam.triangles, labels = c('A', 'B'), nrow = 1, rel_widths = c(3, 2), label_size = 25, greedy = F)
ggsave(file = here('figures', 'main_figures', 'conceptual_plot.png'), height = 8, width = 20)
```
