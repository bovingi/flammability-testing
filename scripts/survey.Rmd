...
species_traits <- data.frame(species, `Drought Tolerant`, Edible, `Wildlife Habitat`, Flowers, Fragrant) %>% 
  pivot_longer(cols = !c(species), names_to = 'trait', values_to = 'rep')

repr.plot <- ggplot(species_traits, aes(x = trait, y = species, fill= rep)) + 
  geom_tile() +
  scale_fill_gradient(low="#ADB6B6B2", high="#ED0000B2") +
...
## Arranged
```{r}
ggarrange(repr.plot, traits.survey.plot,
          labels = c('A', 'B'),
