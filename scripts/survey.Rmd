...
# Plant species
```{r}
species_list_data <- data %>% 
  dplyr::select(plant1, plant2, plant3, plant4, plant5) %>% 
  pivot_longer(cols = c(plant1, plant2, plant3, plant4, plant5)) %>% 
  dplyr::select(-name) %>% 
  na.omit()
write.csv(species_list_data, here('data', 'species_list_data.csv'))
```
