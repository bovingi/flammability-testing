...
plants_text <- data.frame(trait_names = c('Wildlife Habitat', 'Fragrant', 'Flowers', 'Edible', 'Drought Tolerant'),
                          y.lab = c(11, 5, 7, 14.5, 25),
                          lab.color = c(rep('black', 4), 'white'))

traits.survey.plot <- plants %>% 
  ggplot(aes(x = trait_names)) +
  geom_bar(fill = 'gray30') +
  geom_text(data = plants_text, aes(x = trait_names, label = trait_names, y = y.lab, color = lab.color),
  ...
  theme(legend.position = 'none',
        ...
        plot.margin = unit(c(.25, .15, 1.1, .15), "cm"))
traits.survey.plot
```
...
cultural <-  melted %>% 
  ...
  theme(legend.position="none", 
        axis.title.y = element_text(face = 'bold', size = 18, vjust = 4, hjust = 0.3),
...
reg <-  melted %>% 
  ...
  #scale_fill_brewer(palette="BrBG")+
  theme(legend.position="none", 
        axis.title.y = element_text(face = 'bold', size = 18, vjust = 1),
        axis.text.y = element_text(size = 14),
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank(),
...
prov <-  melted %>% 
  ...
  coord_flip() + 
  labs(x = "Provisioning", y = "Percentage") +
  scale_fill_manual(values = c("#595D7C", "#797F91", "#ADB6B2", "#C17C79", "#D24D4B")) +
  #scale_fill_brewer(palette="BrBG")+
  theme(legend.position="none", 
        axis.title.y = element_text(face = 'bold', size = 18, vjust = 2.8),
        axis.text.y = element_text(size = 14),
        axis.text.x = element_text(size = 16),
...
plot_grid(survey_likert, traits.survey.plot,
          ncol = 2, nrow = 1, rel_widths = c(1, 0.7), labels = c('A', 'B'), label_size = 25)
ggsave(here("figures", 'main_figures', 'arranged_survey_figure.jpg'), height = 6, width = 17)
```
