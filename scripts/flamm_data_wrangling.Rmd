...
For 13 of the 311 tests, the datalogger was either turned off or not working. The below chunk of code is meant to look at these 13 tests to ensure that removing them will not impact the overarching analysis
```{r}
...
Filtering out tests with start temp. < 100 degrees C (note: this threshold may change); this step also gets rid of -Inf values... this may not be what we want (will check with KF)
```{r}
#flam.df <- flam.df %>%
  #filter(start_temp > 100)

# Note: for dataframe w/o filtering by 100 degrees threshold and w/ -Inf values (i.e., when the datalogger was turned off) in, run the following two lines of code instead of the two lines above and then run the rest of the below code as normal except for the 'saving .csv' chunk (see that chunk)
```
...
```{r, warning = FALSE}
# Splitting flam.df into two dataframes -- one with -Inf values for temp. and heat flux (i.e., datalogger wasn't on) and one without
...
for(i in 1:nrow(flam.df.no.Inf)){
  df <- heatflux.df %>% 
    ...
    filter(time >= flam.df.no.Inf$start_time[i] & time <= flam.df.no.Inf$end_time[i]) %>% #again, index row, select columns
    mutate(time = as.numeric(time))
  
# Loess regressions
  loess.mod.CH7 <- loess(CH7 ~ time, data = df, span = 0.08860465)
  df$loess.CH7 <- predict(loess.mod.CH7)
  loess.mod.CH8 <- loess(CH8 ~ time, data = df, span = 0.03069767)
...
flam.df.Inf <- flam.df.Inf %>% 
  mutate(max_heat_flux_loessCH7 = NA, max_heat_flux_loessCH8 = NA, time_at_max_heat_flux_loessCH7 = NA,
         time_at_max_heat_flux_loessCH8 = NA)
...
This is the metric we decided to use as our main heat flux metric based on exploratory analysees (see datalogger_exploration.Rmd); we used CH8 values, as these had less noise and showed a stronger signal when regressed on flammability metrics
```{r}
flam.df <- flam.df %>% 
...
# For the .csv w/o filtering by ambient temp. and w/o filtering out rows w/o datalogger data:
#write_csv(flam.df, here('data', 'processed-data', 'flamm_data_without_filtering_datalogger.csv'))
```
