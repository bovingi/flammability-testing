...
ggplot(flam.df.no.Inf, aes(x = ignition, y = max_temp2)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.1, height = 0.02, alpha = 0.4, aes(color = species)) +
  labs(x = 'Ignition', y = 'Max. Temp. (Duration of Test)', color = "Species") +
  theme_bw()

ggplot(flam_sierra_df, aes(x = as.factor(ignition), y = lower_temp_max)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.1, height = 0.02, alpha = 0.4, aes(color = spp)) +
  labs(title = 'Sierra Flam. Testing', x = 'Ignition', y = 'Max. Temp. (Duration of Test)', color = "Species") +
  theme_bw()
```


# Max Temp Exploratory Figures
```{r}
flam.df %>% 
  select(species, ignition, max_temp, max_temp2, time_at_max_temp, time_at_max_temp2) %>% 
  na.omit() %>% 
  filter(ignition != '0') %>%
  filter(max_temp < 275) %>% 
  ggplot(aes(x = max_temp, y = max_temp2, color = species)) +
  geom_point(size = 3) +
  geom_abline(linetype = 4) +
  geom_smooth(method = 'lm', color = 'black') +
  labs(x = 'Max. Temp. (Flame Duration)', y = 'Max Temp. (Full Test)', color = 'Species', title = 'Max. Temp. Exploration: Calculated for duration of flame vs. full test,\n exluding non-ignitions') + 
  theme_bw()

flam.df %>% 
  select(species, ignition, max_temp2, time_at_max_temp2, time_of_glow_end) %>% 
  na.omit() %>% 
  filter(ignition == '0') %>%
  mutate(delta_max_temp_time = time_of_glow_end - time_at_max_temp2) %>% 
  ggplot(aes(x = delta_max_temp_time)) +
  geom_histogram() +
  labs(x = 'Time of Glow End minus Time of Max. Temp.', title = 'Max. Temp. Exploration: Timing of Max. temp. for non-ignitions #1') + 
  theme_bw()
...
flam.df %>% 
  ...
  filter(ignition == '0') %>%
  mutate(delta_max_temp_time = time_of_glow_end - time_at_max_temp2) %>% 
  ggplot(aes(x = max_temp2, y = delta_max_temp_time, color = species)) +
  geom_point(size = 3) +
  geom_smooth(method = 'lm', color = 'black') +
  labs(x = 'Max. Temp. (full test)', y = 'Time of Glow End minus Time of Max. Temp.', color = 'Species', title = 'Max. Temp. Exploration: Timing of Max. temp. for non-ignitions #2') + 
  theme_bw()
```
