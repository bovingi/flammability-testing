...
BIC(fh.sw) # sig. worse than top models
...
AIC(fh.ssav) # sig. worse than top models
BIC(fh.ssav) # sig. worse than top models
...
AIC(temp.la) # sig. worse than top models
BIC(temp.la) # sig. worse than top models
...
for (j in 1:length(BIC_vec)){
 ...
 glmmLasso(fh ~ species + stem_sav + branching + start_temp + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fh_lasso <- glmmLasso(fh ~ species + stem_sav + branching + start_temp + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
fd_lasso <- glmmLasso(fd ~ species + branching + sample_wt + dmc + leaf_mass_ratio,
 ...
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = fd_lambda_bic, # try both aic-based lambda and bic-based lambda
 switch.NR = TRUE,
 final.re = TRUE)
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(temp_change ~ species + branching + leaf_area + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
lambda[which.min(AIC_vec)]
# 6.158482
tc_lambda <- lambda[which.min(BIC_vec)] # since aic and bic had same results, lambda can be either/or
```

### Model Selection
```{r}
tc_lasso <- glmmLasso(temp_change ~ species + branching + leaf_area + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
coef <- c(0.250, NA, NA, NA, NA, 0.023, 0.384, 0.576, NA, NA, NA,
          NA, 0.220, 0.378, NA, NA, NA, NA, 0.418, 0, NA, NA,
          0.266, NA, NA, NA, NA, NA, NA, 0.123, NA, -0.532, -0.308,
          NA, NA, NA, -0.082, -0.263, NA, NA, 0.758, 0, NA, NA)
...
ggplot(aes(x = predictors, y = flam_metrics, fill = coef)) +
  ...
  theme(legend.position = 'top',
        ...
        axis.text.x = element_text(size = 14, face = 'bold', angle = 90, hjust = 1, vjust = 0.5),
