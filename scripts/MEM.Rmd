...
mem.df <- flamm.df %>% 
  ...
  drop_na(lfm, LMA, sample_wt, leaf_mass_ratio, branching, mpa, start_temp, dmc, leaf_dmc, branch_volume, leaf_area, leaf_sav, dw_sppdev, species)

mem.predictors.df <- mem.df %>% 
  select(lfm, LMA, sample_wt, leaf_mass_ratio, branching, mpa, start_temp, dmc, leaf_dmc, branch_volume, leaf_area, leaf_sav, dw_sppdev, species) # fourteen predictors (note: no ww or dw); removed  thickness, leaf_area, sample_density, branch_volume, and branch_height from initial consideration as more than 14 predictors starts to make the model selection process take a really long time or cause the R session to abort
...
BIC(fh.sw) # similar to top models
...
AIC(fh.mpa) # sig. worse than top models
BIC(fh.mpa) # sig. worse than top models
fh.sav <- lmer(fh ~ stem_sav + (1|plant_id), mem.df)
AIC(fh.sav) # sig. worse than top models
BIC(fh.sav) # sig. worse than top models

# Flame Duration
fd.sw <- lmer(fd ~ sample_wt + (1|plant_id), mem.df)
AIC(fd.sw) # sig. worse than top models
BIC(fd.sw) # sig. worse than top models
...
AIC(fd.dmc) # sig. worse than top models
BIC(fd.dmc) # sig. worse than top models
fd.dw <- lmer(fd ~ dw_sppdev + (1|plant_id), mem.df)
AIC(fd.dw) # sig. worse than top models
BIC(fd.dw) # sig. worse than top models
...
temp.lma <- lmer(temp_change ~ LMA + (1|plant_id), mem.df)
AIC(temp.lma) # sig. worse than top models
BIC(temp.lma) # sig. worse than top models

# Heat Flux
hf.sw <- lmer(heat_flux_change ~ sample_wt + (1|plant_id), mem.df)
AIC(hf.sw) # sig. worse from top models
...
AIC(hf.sav) # sig. worse than top models
BIC(hf.sav) # sig. worse than top models
hf.lma <- lmer(heat_flux_change ~ LMA + (1|plant_id), mem.df)
AIC(hf.lma) # sig. worse than top models
BIC(hf.lma) # sig. worse than top models
```
...
for (j in 1:length(BIC_vec)){
 ...
 glmmLasso(fh ~ species + branching + sample_wt + mpa + stem_sav,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fh_lasso <- glmmLasso(fh ~ species + branching + sample_wt + mpa + stem_sav,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = fh_lambda_bic, # try both aic-based lambda and bic-based lambda
 switch.NR = TRUE,
 final.re = TRUE)
...
```
w/ lfm
Removed variables: stem_sav, lfm, speciesMALLAU and speciesSALAPI are removed from consideration
Significance: sample weight is selected as significant predictor; branching is marginal (p = 0.058)
w/o lfm
Removed variables: stem_sav and speciesSALAPI are removed from consideration
Significance: sppERIKAR is selected as significant predictor; branching is marginal (p = 0.074)
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(fd ~ species + branching + sample_wt + dmc + dw_sppdev,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fd_lasso <- glmmLasso(fd ~ species + branching + sample_wt + dmc + dw_sppdev,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
```
w/ lfm
Variables removed: speciesMALLAU, speciesSALLEU, lfm, dw_sppdev removed from consideration
Significance: none are selected as significant predictors
w/o lfm
Variables removed: speciesMALLAU and dmc removed from consideration
Significance: speciesERIKAR, speciesSALAPI are selected as significant predictors
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(temp_change ~ species + lfm + LMA + branching + sample_wt + start_temp + mpa,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
tc_lasso <- glmmLasso(temp_change ~ species + lfm + LMA + branching + sample_wt + start_temp + mpa,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
summary(tc_lasso)
```
Variables Removed: speciesSALLEU
Significance: start temp, LMA, speciesMALLAU and speciesSALAPI
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(heat_flux_change ~ species + sample_wt + leaf_mass_ratio + LMA + leaf_sav,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}  
hf_lasso <- glmmLasso(heat_flux_change ~ species + sample_wt + leaf_mass_ratio + LMA + leaf_sav,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
```
w/ lfm
Variables removed: speciesERIKAR, speciesMALLAU, lfm, LMA removed
Significance: speciesSALLEU and sample_wt are significant
w/o lfm
Variables removed: speciesSALAPI and leaf sa:v removed
Significance: speciesMALLAU, speciesSALLEU and sample_wt are significant
...
fh.branching.remef <- ggplot(mem.df, aes(x = branching, y = r_fh))  +
  ...
  theme_bw() +
  scale_color_manual(values = c("#42B540B2", "#0099B4B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
...
ggplot(mem.df, aes(x = lfm, y = r_fh))  +
  ...
  stat_cor(label.y = 0.9)+ 
  stat_regline_equation(label.y = 1.2) +
  labs(x = 'Live Fuel Moisture (scaled)', y = 'Flame Height (effects removed, scaled)', color = 'Species') +
  theme_bw() +
  scale_color_manual(values = c("#42B540B2", "#0099B4B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12),
...
Leaf SA:V:
```{r}
fh_mod <- lmer(fh ~ stem_sav + branching + sample_wt + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_fh <- remef(fh_mod, fix = c('sample_wt', 'branching'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = stem_sav, y = r_fh))  +
  geom_point(aes(color = species), alpha = 0.8) +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 0.9)+ 
  stat_regline_equation(label.y = 1.2) +
  labs(x = 'Stem SA:V (scaled)', y = 'Flame Height (effects removed, scaled)', color = 'Species') +
  theme_bw() +
  scale_color_manual(values = c("#42B540B2", "#0099B4B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12),
        legend.title = element_text(face = 'bold', size = 15))
ggsave(here('figures', 'MEM', 'REMEF', 'stem_sav.vs.fh.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = stem_sav, y = r_fh))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 0.9)+ 
  stat_regline_equation(label.y = 1.2) +
  labs(x = 'Stem SA:V (scaled)', y = 'Flame Height (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'stem_sav.vs.fh.spp.png'), width = 8, height = 5.5)
```
...
DW spp dev:
```{r}
fd_mod <- lmer(fd ~ species*dw_sppdev + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs
...
# With REMEF
fd.dw.remef <- ggplot(mem.df, aes(x = dw_sppdev, y = r_fd))  +
  geom_point(aes(color = species), alpha = 0.8) +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 3.95)+ 
  stat_regline_equation(label.y = 4.55) +
  labs(x = 'Dry Weight Deviation (scaled)', y = 'Flame Duration', color = 'Species') +
  theme_bw() +
  scale_color_manual(values = c("#42B540B2", "#0099B4B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12),
        legend.title = element_text(face = 'bold', size = 15))
fd.dw.remef
ggsave(here('figures', 'MEM', 'REMEF', 'dw.vs.fd.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = dw_sppdev, y = r_fd))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
  labs(x = 'Dry Weight Deviation (scaled)', y = 'Flame Duration (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'dw.vs.fd.spp.png'), width = 8, height = 5.5)

# Interaction with species visualized
fd.dwint.remef <- ggplot(mem.df, aes(x = dw_sppdev, y = r_fd, color = species))  +
  geom_point(aes(color = species), alpha = 0.8) +
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        ...
        legend.title = element_text(face = 'bold', size = 15))
fd.dwint.remef
ggsave(here('figures', 'MEM', 'REMEF', 'dwxspp.vs.fd.png'), width = 8, height = 5.5)
```

Sample Weight:
```{r}
fd_mod <- lmer(fd ~ sample_wt + dw_sppdev*species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

summary(fd_mod)

mem.df$r_fd <- remef(fd_mod, fix = c('dw_sppdev'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height
...
LMA:
```{r}
hf_mod <- lmer(heat_flux_change ~ LMA + species + sample_wt + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs
...
hf.lma.remef <- ggplot(mem.df, aes(x = LMA, y = r_hf))  +
  ...
  labs(x = 'LMA (scaled)', y = 'Heat Flux Change', color = 'Species') +
  theme_bw() +
...
ggplot(aes(x = LMA, y = r_hf))  +
  ...
  stat_cor(label.y = 1.25)+ 
  stat_regline_equation(label.y = 1.5) +
  labs(x = 'LMA (scaled)', y = 'Heat Flux Change (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
...
LFM:
```{r}
temp_mod <- lmer(temp_change ~ start_temp*sample_wt + lfm + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_temp <- remef(temp_mod, fix = c('start_temp', 'sample_wt', 'start_temp:sample_wt'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = lfm, y = r_temp))  +
  geom_point(aes(color = species), alpha = 0.8) +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
  labs(x = 'Live Fuel Moisture (scaled)', y = 'Temp. Change (effects removed, scaled)', color = 'Species') +
  theme_bw() +
  scale_color_manual(values = c("#42B540B2", "#0099B4B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  theme(legend.title = element_text(face = 'bold', size = 15),
        axis.title = element_text(face = 'bold', size = 15),
...
## Arranged
```{r}
ggarrange(fh.branching.remef, fd.dw.remef, temp.branching.remef, hf.lma.remef,
          labels = c('A', 'B', 'C', 'D'), 
...
predictors <- c('lfm', 'LMA', 'sample_wt', 'leaf_mass_ratio', 'branching', 'mpa', 'start_temp', 'dmc', 'leaf_sav', 'stem_sav', 'dw_sppdev', 'species')
...
AIC(fd.int.mod.list[[59]]) 
fd.int.mod.list[[60]] <- lmer(fd ~ lfm*sample_wt + branching + species + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[60]])
fd.int.mod.list[[61]] <- lmer(fd ~ species*dw_sppdev + branching + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[61]]) 
fd.int.mod.list[[62]] <- lmer(fd ~ species*dw_sppdev + sample_wt + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[62]])
fd.int.mod.list[[63]] <- lmer(fd ~ species*dw_sppdev + lfm + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[63]])
fd.int.mod.list[[64]] <- lmer(fd ~ species*dw_sppdev + mpa + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[64]])
fd.int.mod.list[[65]] <- lmer(fd ~ species*dw_sppdev + dmc + (1|plant_id), data = mem.df)
AIC(fd.int.mod.list[[65]])
```
...
temp.int.mod.list[[58]] <- lmer(temp_change ~ leaf_sav*start_temp + species + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[58]])
temp.int.mod.list[[59]] <- lmer(temp_change ~ start_temp*sample_wt + lfm + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[59]]) 
temp.int.mod.list[[60]] <- lmer(temp_change ~ lfm*start_temp + sample_wt + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[60]])
temp.int.mod.list[[61]] <- lmer(temp_change ~ leaf_sav*start_temp + sample_wt + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[61]])
temp.int.mod.list[[62]] <- lmer(temp_change ~ start_temp*sample_wt + branching + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[62]]) 
temp.int.mod.list[[63]] <- lmer(temp_change ~ start_temp*sample_wt + LMA + (1|plant_id), data = mem.df)
AIC(temp.int.mod.list[[63]]) 
```
...
AIC(hf.int.mod.list[[63]])
hf.int.mod.list[[64]] <- lmer(heat_flux_change ~ start_temp*sample_wt + lfm + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[64]])
hf.int.mod.list[[65]] <- lmer(heat_flux_change ~ dw_sppdev*species + leaf_mass_ratio + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[65]])
hf.int.mod.list[[66]] <- lmer(heat_flux_change ~ dw_sppdev*species + start_temp + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[66]])
hf.int.mod.list[[67]] <- lmer(heat_flux_change ~ dw_sppdev*species + lfm + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[67]])
hf.int.mod.list[[68]] <- lmer(heat_flux_change ~ dw_sppdev*species + sample_wt + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[68]])
```

# Nice Model Selection Results Table
```{r}
predictors <- rep(colnames(mem.predictors.df), 4)
flam_metrics <- c(rep('Flame Height', ncol(mem.predictors.df)),
                  rep('Flame Duration', ncol(mem.predictors.df)),
                  rep('Temp. Change', ncol(mem.predictors.df)),
                  rep('Heat Flux Change', ncol(mem.predictors.df)))
coef <- c(NA, NA, 0.445, NA, 0.165, NA, NA, NA, NA, NA, NA, NA, NA, 0,
          NA, NA, NA, NA, NA, NA, NA, 0.311, NA, NA, NA, NA, 0.347, 0,
          NA, NA, 0.21, NA, 0.253, NA, -0.579, NA, NA, NA, NA, NA, NA, NA,
          NA, NA, 0.806, 0.244, NA, NA, NA, NA, NA, NA, NA, NA, NA, 0)
p.val <- c(NA, NA, '***', NA, '*', NA, NA, NA, NA, NA, NA, NA, NA, '*',
          NA, NA, NA, NA, NA, NA, NA, '*', NA, NA, NA, NA, '***', '***',
          NA, NA, '**', NA, '***', NA, '***', NA, NA, NA, NA, NA, NA, NA,
          NA, NA, '***', ' ', NA, NA, NA, NA, NA, NA, NA, NA, NA, '*')
woo <- data.frame(predictors, flam_metrics, coef, p.val) %>% 
  filter(predictors != c('leaf_dmc', 'stem_dmc')) %>% 
  mutate(predictors = case_when(
    predictors == 'lfm' ~ 'Live Fuel Moisture',
    predictors == 'LMA' ~ 'Leaf Mass per Area',
    predictors == 'sample_wt' ~ 'Sample Weight',
    predictors == 'leaf_mass_ratio' ~ 'Leaf Mass Ratio',
    predictors == 'branching' ~ 'Branching',
    predictors == 'mpa' ~ 'Water Potential',
    predictors == 'start_temp' ~ 'Starting Temp.',
    predictors == 'dmc' ~ 'Dry Matter Content',
    predictors == 'leaf_sav' ~ 'Leaf Surface Area:Volume',
    predictors == 'stem_sav' ~ 'Stem Surface Area:Volume',
    predictors == 'dw_sppdev' ~ 'Dry Wt. (Species Deviation)',
    predictors == 'species' ~ 'Species'
  ))

woo %>%
  na.omit() %>% 
ggplot(aes(y = predictors, x = flam_metrics, color = coef)) +
  geom_point(size = 20, shape = 15) +
  labs(y = 'Predictors', x = 'Flam. Metrics', color = 'Coefficient') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12),
        legend.title = element_text(face = 'bold', size = 15))
...
ggplot(aes(x = predictors, y = flam_metrics, fill = coef)) +
  ...
  geom_text(aes(x = predictors, y = flam_metrics, label = p.val), color = 'white', fontface = 'bold', size = 6) +
...
ggsave(here('figures', 'MEM', 'example.mem.results.png'), height = 6.5, width = 9)
```
...
concept.remef.df <- flamm.df %>% 
  ...
  drop_na(lfm, sample_wt, leaf_mass_ratio, branching, mpa, start_temp, dmc, leaf_dmc, leaf_sav, dw_sppdev, species)

# mixed effects models
fh_c_mod <- lmer(fh ~ branching + sample_wt + lfm + leaf_mass_ratio + species + start_temp + leaf_sav + (1|plant_id), data = concept.remef.df, REML = F) # remef is also sensitive to NAs
fd_c_mod <- lmer(fd ~ branching + sample_wt + lfm + leaf_mass_ratio + species + start_temp + leaf_sav + (1|plant_id), data = concept.remef.df, REML = F) # remef is also sensitive to NAs
temp_c_mod <- lmer(temp_change ~ branching + sample_wt + lfm + leaf_mass_ratio + species + start_temp + leaf_sav + (1|plant_id), data = concept.remef.df, REML = F) # remef is also sensitive to NAs
hf_c_mod <- lmer(heat_flux_change ~ branching + sample_wt + lfm + leaf_mass_ratio + species + start_temp + leaf_sav + (1|plant_id), data = concept.remef.df, REML = F) # remef is also sensitive to NAs
