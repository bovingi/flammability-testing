...
AIC(fd.lmr) # sig. worse than top models
BIC(fd.lmr) # sig. worse than top models
...
AIC(hf.sw) # similar to top models
...
for (j in 1:length(BIC_vec)){
 ...
 glmmLasso(fh ~ species + branching + sample_wt + lfm,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fh_lasso <- glmmLasso(fh ~ species + branching + sample_wt + lfm,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(fd ~ species + branching + sample_wt + dmc + leaf_mass_ratio,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fd_lasso <- glmmLasso(fd ~ species + branching + sample_wt + dmc + leaf_mass_ratio,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(temp_change ~ species + lfm + branching + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
tc_lasso <- glmmLasso(temp_change ~ species + lfm + branching + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(heat_flux_change ~ species + sample_wt + branch_volume + lfm,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}  
hf_lasso <- glmmLasso(heat_flux_change ~ species + sample_wt + branch_volume + lfm,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
ggplot(mem.df, aes(x = sample_wt, y = r_fh))  +
  ...
  theme_bw() +
  scale_color_manual(values = c("#ED0000B2", "#42B540B2", "#0099B4B2",  "#FDAF91B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12),
        legend.title = element_text(face = 'bold', size = 15))
...
## Arranged
```{r}
ggarrange(fh.branching.remef, fd.dmc.remef, temp.branching.remef, hf.lma.remef,
          labels = c('A', 'B', 'C', 'D'), 
...
predictors <- c('lfm', 'LMA', 'sample_wt', 'leaf_mass_ratio', 'branching', 'mpa', 'start_temp', 'dmc', 'leaf_dmc', 'branch_volume', 'leaf_area', 'leaf_sav', 'dw_sppdev', 'species')
...
AIC(hf.int.mod.list[[58]])
hf.int.mod.list[[59]] <- lmer(heat_flux_change ~ LMA*sample_wt + branch_volume + (1|plant_id), data = mem.df)
...
hf.int.mod.list[[62]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + lfm + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[62]])
hf.int.mod.list[[63]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + species + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[63]])
hf.int.mod.list[[64]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + branch_volume + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[64]])
hf.int.mod.list[[65]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + LMA + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[65]])
hf.int.mod.list[[66]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + start_temp + (1|plant_id), data = mem.df)
AIC(hf.int.mod.list[[66]])
hf.int.mod.list[[67]] <- lmer(heat_flux_change ~ leaf_dmc*sample_wt + lfm + species + (1|plant_id), data = mem.df)
...
## Figure
```{r}
pred_vec <- c('branching', 'dmc', 'dmc:sample_wt', 'lfm', 'lfm:sample_wt', 'LMA', 'LMA:sample_wt', 'sample_wt', 'species', 'start_temp', 'start_temp:sample_wt')
predictors <- rep(pred_vec, 4)
flam_metrics <- c(rep('Flame\nHeight', length(pred_vec)),
                  rep('Flame\nDuration', length(pred_vec)),
                  rep('Temperature\nChange', length(pred_vec)),
                  rep('Heat Flux\nChange', length(pred_vec)))
coef <- c(0.268, NA, NA, 0.008, 0.351, NA, NA, 0.577, NA, NA, NA,
          NA, 0.205, 0.360, NA, NA, NA, NA, 0.385, 0, NA, NA,
          0.270, NA, NA, NA, NA, NA, NA, 0.120, NA, -0.505, -0.289,
          NA, NA, NA, NA, NA, -0.386, -0.009, 0.832, 0, NA, NA)
...
woo <- data.frame(predictors, flam_metrics, coef, p.val) %>% 
  ...
  mutate(predictors = case_when(
    ...
    predictors == 'LMA:sample_wt' ~ 'LMA x\nSample Weight',
    predictors == 'start_temp:sample_wt' ~ 'Starting Temp x\nSample Weight'
  )) %>% 
  mutate(predictors = as.factor(predictors)) %>% 
  mutate(predictors = fct_relevel(predictors, c('Live Fuel\n Moisture', 'Leaf Mass\n per Area', 'Sample Weight', 'Branching', 'Starting\n Temperature', 'Dry Matter\n Content', 'Species', 'DMC x\nSample Weight', 'LFM x\nSample Weight', 'LMA x\nSample Weight', 'Starting Temp x\nSample Weight'))) %>% 
  mutate(flam_metrics = as.factor(flam_metrics)) %>% 
  mutate(predictors = fct_relevel(predictors, c('Flame\nHeight', 'Flame\nDuration', 'Temperature\nChange', 'Heat Flux\nChange')))
...
ggplot(aes(x = predictors, y = flam_metrics, fill = coef)) +
  ...
  scale_fill_gradient2(low = 'darkred', mid = "gray80", high = 'darkblue', midpoint = 0, na.value = 'white', limits = c(-0.85, 0.85)) +
...
ggsave(here('figures', 'MEM', 'example.mem.results.png'), height = 5.5, width = 9)
```
