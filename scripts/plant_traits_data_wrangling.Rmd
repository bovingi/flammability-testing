...
```{r}
# load in data
leaf_data <- read_csv(here('raw-data', 'plant-traits', 'leaf_data_flamm.csv')) %>%
  clean_names() %>%
  slice_head(n = 467) %>%
  select(-x12)
...
```{r}
# load in data
leaf_area <- read_csv(here('raw-data', 'plant-traits', 'leaf_area_flamm.csv')) %>%
  clean_names() %>%
  slice_head(n = 314) %>%
...
```{r}
# load in data
stem_leaf_ratio <- read_csv(here('raw-data', 'plant-traits', 'stemleaf_massratio_flamm.csv')) %>%
  clean_names() %>%
  slice_head(n = 92) %>%
...
```{r}
# load in data
stem_data <- read_csv(here('raw-data', 'plant-traits', 'stem_data_flamm.csv')) %>%
  clean_names() %>%
  slice_head(n = 371) 
...
```{r}
cup_weights_data <- read_csv(here('raw-data', 'plant-traits', 'cup_weights.csv')) %>% 
  slice_head(n = 82)  %>% 
