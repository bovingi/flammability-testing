...
```{r}
library(bayestestR) # For area_under_curvee
voc_data <- voc_data %>% 
  group_by(species, method, leaf_mass) %>% 
  mutate(auc_voc = area_under_curve(time, voc), # Area under curve
