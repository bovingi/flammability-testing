...
cup_weights_data <- read_csv(here('data', 'raw-data', 'plant-traits', 'cup_weights.csv')) %>% 
  slice_head(n = 82)  %>% 
  rename(cup_mass = mass, dry_cup_mass = mass_post_drying)
...
voc_data_avgs <- voc_data %>% 
  group_by(species) %>% 
  summarise(auc_voc = mean(auc_voc), avg_voc = mean(avg_voc))
```
...
leaf_stem_df <- leaf_stem_df %>% 
  mutate(lfm = ifelse(is.na(stem_wet_mass),leaf_lfm, 100*((stem_wet_mass + leaf_wet_mass) - (stem_dry_mass + leaf_dry_mass))/(stem_dry_mass + leaf_dry_mass)))
...
na <- c(rep(999999, 3)) # so that IRIDOU data does not get lost in the merge
voc_data_avgs_nas <- data.frame(species, na, na)
...
main_traits_df <- main_traits_df %>% 
  ...
  mutate(auc_voc = ifelse(auc_voc > 99999, NA, auc_voc)) %>% 
  mutate(avg_voc = ifelse(avg_voc > 99999, NA, avg_voc))
```

## Calculating LMA, SLA
Note that for ERIKAR P4-P10 and ARTCAL, leaf wet mass was for all of the leaves rather than just the five used in the leaf scans, hence why those values are NA below
```{r}
main_traits_df <- main_traits_df %>% 
  mutate(LMA = ifelse(species == 'ARTCAL' | species == 'ERIKAR' & plant %in% c(4:10), NA, leaf_wet_mass/leaf_area),
         SLA = ifelse(species == 'ARTCAL' | species == 'ERIKAR' & plant %in% c(4:10), NA, leaf_area/leaf_wet_mass))
```
