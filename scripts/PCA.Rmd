...
figure_pca_quant2 <- (traits.df_pca[, c('species','lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'stem_sav', 'dmc')]) %>%  
 transmute("      LFM" = lfm,  # added spaces to adjust label position
           ...
           "Sample Weight\n" = sample_wt,
           "                Dry Matter Content" = dmc, # added spaces to adjust label position
           "Leaf SA:V" = leaf_sav,
           "Stem SA:V\n" = stem_sav) %>% 
  na.omit()
...
PCA_metrics2 <- df.v %>% 
  filter(Variables %in% c("      LFM", "Water Potential", "Sample Density\n",
           "LMA", "Thickness", "Branching", "\nLeaf Ratio",
           "Sample Weight\n", "                Dry Matter Content", "Leaf SA:V",
           "Stem SA:V\n"))
...
traits.pca <- ggplot(PCAvalues2, aes(x = PC1, y = PC2)) +
  ...
  geom_segment(data = PCA_metrics2, aes(x = 0, y = 0, 
    ...
    linewidth = .9, alpha = 0.9,
     color = "black") +
  annotate("text", 
          x = (PCA_metrics2$PC1 * 1.35), 
          y = (PCA_metrics2$PC2 * 1.25),
...
```{r}
pc_labels <- c('PC1 (-FH)', 'PC2 (-\U0394Temp)', 'PC3 (FD)', 'PC1 (DMC, -LFM,)', 'PC2 (-Br., -MPa)', 'PC3 (L. SA:V, -Thick.)')
