...
```{r}
flamm.df <- flamm.df %>% 
  filter(species %in% c('CEAGRI', 'ERIKAR', 'MALLAU', 'SALAPI', 'SALLEU')) %>% 
  filter(ignition != '0') %>% 
...
traits.df_pca <- flamm.df %>% 
  filter(ignition != 0) %>% # No non-ignitions
   select(id, species, lfm, mpa, ww_flam_sample, dw_flam_sample, LMA, thickness, branching, leaf_mass_ratio, sample_wt, branch_volume) %>% 
   drop_na(species, lfm, mpa, ww_flam_sample, dw_flam_sample, LMA, thickness, branching, leaf_mass_ratio, sample_wt, branch_volume)

figure_pca_quant2 <- (traits.df_pca[, c('species','lfm', 'mpa', 'ww_flam_sample', 'dw_flam_sample', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'branch_volume')]) %>%  
 transmute("Live Fuel Moisture" = lfm, #using spaces to shift labels over instead of \t since \t is leading to little blank squares popping up on the plot
           "Water Potential" = mpa,
           "Wet Weight (g)" = ww_flam_sample,
           "Dry Weight (g)" = dw_flam_sample,
...
figure_pca_cat2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'ww_flam_sample', 'dw_flam_sample', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'branch_volume')]) %>% 
  na.omit() %>% 
  select(species) %>% 
  transmute("Species" = species)
...
figure_pca2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'ww_flam_sample', 'dw_flam_sample', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'branch_volume')]) %>%  
  na.omit() 
```
...
#Group by flam. metric: 
PCA_metrics2 <- df.v %>% 
  filter(Variables %in% c("Live Fuel Moisture", "Water Potential", "Wet Weight (g)", 
                          "Dry Weight (g)", "Leaf Mass per Area", "Thickness (mm)",
...
pca_df2 <- pca_df2 %>% 
  select(-PC5, -PC6, -PC7, -PC8, -PC9, -PC10, -species) %>%  # Not interested in PC5 through PC10 (>80% of variance explained by first four PCs); species variable already in pca_df
...
flamm.df_pca <- flamm.df %>% 
  filter(ignition != 0) %>% # No non-ignitions
   select(fh, fd, temp_change, heat_flux_change, lfm, mpa, ww_flam_sample, LMA, branching, species, id) %>% 
   drop_na(fh, fd, temp_change, heat_flux_change, lfm, mpa, ww_flam_sample, LMA, branching, species)

figure_pca_quant <- (flamm.df_pca[, c("fh", "fd", "temp_change", "heat_flux_change", "species",'lfm', 'mpa', 'ww_flam_sample', 'LMA', 'branching')]) %>%  
 transmute("Flame Duration" = fd, #using spaces to shift labels over instead of \t since \t is leading to little blank squares popping up on the plot
           "Flame Height" = fh,
           "Temp Change" = temp_change,
           "Heat Flux Change" = heat_flux_change,
...
figure_pca_cat <- (flamm.df_pca[, c("fh", "fd", "temp_change", "heat_flux_change", "species",'lfm', 'mpa', 'ww_flam_sample', 'LMA', 'branching')]) %>%  
  na.omit() %>% 
  select(species) %>% 
  transmute("Species" = species)
...
figure_pca <- (flamm.df_pca[, c("fh", "fd", "temp_change", "heat_flux_change", "species",'lfm', 'mpa', 'ww_flam_sample', 'LMA', 'branching')]) %>%  
  na.omit() 
```
...
flamm.df_pca <- flamm.df %>% 
  filter(ignition != 0) %>% # No non-ignitions
   select(fh, fd, temp_change, heat_flux_change, species) %>% 
   drop_na(fh, fd, temp_change, heat_flux_change, species)
...
ggplot(PCAvalues, aes(x = PC1, y = PC2)) +
  ...
  xlab(u.axis.labs[1]) + 
   ylab(u.axis.labs[2]) +
  coord_equal()
ggsave(here('figures', 'PCA', 'Flamm.PCA4.noPFG.all.ignitions.png'), height = 7, width = 9)
```
...
tab_pca(pcobj, rotation = 'varimax',
  ...
  string.pov = "Proportion of Variance",
  string.cpov = "Cumulative Proportion",
  file = here('figures', 'PCA', 'Flamm.PC.Table4.noPFG.all.ignitions.html')) 
```