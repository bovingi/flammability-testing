...
```{r}
pca_df <- pca_df %>% 
  select(-PC5) %>%  # Not interested in PC5
...
pca_df2 <- pca_df2 %>% 
  select(-PC5, -PC6, -PC7, -PC8, -PC9, -PC10, -species) %>%  # Not interested in PC5 through PC10; species in pca_df
...
## Scatterplots
```{r}
pc_labels <- c('PC1 (Flamm.)', 'PC2 (Flamm.)', 'PC3 (Flamm.)', 'PC4 (Flamm.)', 'PC1 (Traits)', 'PC2 (Traits)', 'PC3 (Traits)', 'PC4 (Traits)')
...
pca_df_main %>% 
  ...
  ggplot(aes(x = traits_pc_score, y = flamm_pc_score)) +
    geom_point(alpha = 0.7, color = 'gray30') +
