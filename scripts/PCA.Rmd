...
flam.pca <- ggplot(data = PCAvalues, aes(x = PC1, y = PC2)) +
  ...
  #scale_color_lancet() + # no ARTCAL, so manually defining color palette # no ARTCAL, so manually defining color palette
...
traits.df_pca <- flamm.df %>% 
  filter(ignition != 0) %>% # No non-ignitions
   select(id, species, lfm, mpa, sample_density, LMA, thickness, branching, leaf_mass_ratio, sample_wt, leaf_sav, stem_sav, dmc) %>% 
   drop_na(species, lfm, mpa, sample_density, LMA, thickness, branching, leaf_mass_ratio, sample_wt, leaf_sav, stem_sav, dmc)

figure_pca_quant2 <- (traits.df_pca[, c('species','lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'stem_sav', 'dmc')]) %>%  
 transmute("LFM" = lfm,
           "Water Potential" = mpa,
...
figure_pca_cat2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'stem_sav', 'dmc')]) %>% 
  na.omit() %>% 
  select(species) %>% 
  transmute("Species" = species)
...
figure_pca2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'stem_sav', 'dmc')]) %>%  
  na.omit() 
```
...
#Group by flam. metric: 
PCA_metrics2 <- df.v %>% 
  filter(Variables %in% c("LFM", "Water Potential", "Sample Density",
           "LMA", "Thickness", "Branching", "Leaf Ratio",
           "Sample Weight", "Dry Matter Content", "Leaf SA:V",
           "Stem SA:V"))
...
traits.pca <- ggplot(PCAvalues2, aes(x = PC1, y = PC2)) +
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), # Make background white and border black
          ...
          axis.title = element_text(face = 'bold', size = 15)) +
    scale_color_manual(values = c("#00468BB2", "#42B540B2", "#0099B4B2", "#925E9FB2", "#FDAF91B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
  #scale_color_lancet() + # no ARTCAL, so manually defining color palette
  xlab(u.axis.labs[1]) + 
  ylab(u.axis.labs[2])# +
  #coord_equal() #remove hash if you want to make coordinates equal; for arranging figure, looks best if they're not
traits.pca
...
### Stand-alone
```{r}
pc_labels <- c('PC1 (-FH)', 'PC2 (-Temp. Change)', 'PC3 (FD)', 'PC4 (-HF Change)', 'PC1 (DMC, -LFM, -LMR)', 'PC2 (-Branching, -MPa)', 'PC3 (Leaf SA:V, -Thickness)', 'PC4 (-Sample Weight, -LMA)')
...
pca_df_main %>% 
  ...
  ggplot(aes(x = traits_pc_score, y = flamm_pc_score)) +
    ...
    stat_cor(label.y.npc = 'top', label.x.npc = 'left', size = 2.5)+ 
...
ggsave(here('figures', 'PCA', 'PC.Regressions.png'), height = 8, width = 12)
```

### For Arranged Main Figure
```{r}
pc_labels <- c('PC1 (-FH)', 'PC2 (-Temp. Change)', 'PC3 (FD)', 'PC1 (DMC, -LFM, -LMR)', 'PC2 (-Branching, -MPa)', 'PC3 (Leaf SA:V, -Thickness)')
...
pc.scatterplot <- pca_df_main %>% 
  ...
  ggplot(aes(x = traits_pc_score, y = flamm_pc_score)) +
    geom_point(alpha = 0.5, aes(color = species)) +
    geom_smooth(method = 'lm', color = 'black', alpha = 0.75) +
    stat_cor(label.y.npc = 'top', label.x.npc = 'left', size = 3)+ 
    #stat_regline_equation(label.y.npc = 'bottom', label.x.npc = 'left', size = 3) +
    facet_grid(flamm_pc ~ traits_pc, scales = 'free', labeller = labeller(flamm_pc = pc_labels, traits_pc = pc_labels)) +
...
ggplot(PCAvalues, aes(x = PC1, y = PC2)) +
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), # Make background white and border black
          ...
          legend.text = element_text(face = 'italic', size = 13),
          axis.title = element_text(face = 'bold', size = 15)) +
    scale_color_manual(values = c("#00468BB2", "#42B540B2", "#0099B4B2", "#925E9FB2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) + 
    #scale_color_lancet() + # no ARTCAL, so manually defining color palette 
    geom_path(data = circle, color = "black",  size = 1/2, alpha = 1/3) + 
    xlab(u.axis.labs[1]) + 
    ylab(u.axis.labs[2]) +
    coord_equal()
ggsave(here('figures', 'PCA', 'manual_ignitions', 'Flamm.PCA.png'), height = 7, width = 9)
```
...
tab_pca(pcobj, rotation = 'varimax',
  ...
  string.pov = "Proportion of Variance",
  string.cpov = "Cumulative Proportion",
  file = here('figures', 'PCA', 'manual_ignitions', 'Flamm.PC.Table.html')) 
```
...
ggplot(PCAvalues2, aes(x = PC1, y = PC2)) +
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), # Make background white and border black
    ...
    ylab(u.axis.labs[2]) +
    coord_equal()
ggsave(here('figures', 'PCA', 'manual_ignitions', 'Traits.PCA.png'), height = 7, width = 9)
```
...
tab_pca(pcobj2, rotation = 'varimax',
  ...
  string.pov = "Proportion of Variance",
  string.cpov = "Cumulative Proportion",
  file = here('figures', 'PCA', 'manual_ignitions', 'Traits.PC.Table.html')) 
```
...
ggsave(here('figures', 'PCA', 'manual_ignitions', 'PC.Regressions.png'), height = 8, width = 10)
```
### -*-*-*-*-*-
...
ggplot(PCAvalues, aes(x = PC1, y = PC2)) +
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), # Make background white and border black
    ...
    ylab(u.axis.labs[2]) +
    coord_equal()
ggsave(here('figures', 'PCA', 'natural_ignitions', 'Flamm.PCA.png'), height = 7, width = 9)
```
...
tab_pca(pcobj, rotation = 'varimax',
  ...
  string.pov = "Proportion of Variance",
  string.cpov = "Cumulative Proportion",
  file = here('figures', 'PCA', 'natural_ignitions', 'Flamm.PC.Table.html')) 
```
...
ggplot(PCAvalues2, aes(x = PC1, y = PC2)) +
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), # Make background white and border black
    ...
    ylab(u.axis.labs[2]) +
    coord_equal()
ggsave(here('figures', 'PCA', 'natural_ignitions', 'Traits.PCA.png'), height = 7, width = 9)
```
...
tab_pca(pcobj2, rotation = 'varimax',
  ...
  string.pov = "Proportion of Variance",
  string.cpov = "Cumulative Proportion",
  file = here('figures', 'PCA', 'natural_ignitions', 'Traits.PC.Table.html')) 
```
...
pca_df_main %>% 
  ...
  ggplot(aes(x = traits_pc_score, y = flamm_pc_score)) +
    ...
    theme(axis.title = element_text(size = 16, face = 'bold'),
          axis.text = element_text(size = 13),
          strip.text = element_text(size = 13, face = 'bold'))
ggsave(here('figures', 'PCA', 'natural_ignitions', 'PC.Regressions.png'), height = 7, width = 9)
```