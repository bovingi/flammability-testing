...
```{r}
flamm.df <- flamm.df %>% 
  filter(species %in% c('CEAGRI', 'ERIKAR', 'IRIDOU', 'MALLAU', 'SALAPI', 'SALLEU')) %>% 
  filter(ignition != '0') %>% 
...
figure_pca_quant <- (flamm.df_pca[, c("fh", "fd", "pfg", "temp_change", "heat_flux_change", "species")]) %>%  
...
figure_pca_cat <- (flamm.df_pca[, c("fh", "fd", "pfg", "temp_change", "heat_flux_change", "species")]) %>%  
  na.omit() %>% 
  select(species) %>% 
...
figure_pca <- (flamm.df_pca[, c("fh", "fd", "pfg", "temp_change", "heat_flux_change", "species")]) %>%  
  na.omit() 
```
