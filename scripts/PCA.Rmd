...
```{r}
flamm.df <- flamm.df %>% 
  filter(species %in% c('ARTCAL', 'CEAGRI', 'ERIKAR', 'MALLAU', 'SALAPI', 'SALLEU', 'IRIDOU', 'ARCDEN', 'HETARB')) %>% 
  filter(ignition != '0') %>% 
...
flam.pca <- ggplot(data = PCAvalues, aes(x = PC1, y = PC2)) +
  ...
  scale_color_lancet() + # if no ARTCAL, manually define color palette 
...
traits.df_pca <- flamm.df %>% 
  filter(ignition != 0) %>% # No non-ignitions
   select(id, species, lfm, mpa, sample_density, LMA, thickness, branching, leaf_mass_ratio, sample_wt, leaf_sav, dmc) %>% 
   drop_na(species, lfm, mpa, sample_density, LMA, thickness, branching, leaf_mass_ratio, sample_wt, leaf_sav, dmc)

figure_pca_quant2 <- (traits.df_pca[, c('species','lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'dmc')]) %>%  
 transmute("\nLFM" = lfm,  # added spaces to adjust label position
           "Water Potential" = mpa,
...
figure_pca_cat2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'dmc')]) %>% 
  na.omit() %>% 
  select(species) %>% 
  transmute("Species" = species)
...
figure_pca2 <- (traits.df_pca[, c('species', 'lfm', 'mpa', 'sample_density', 'LMA', 'thickness', 'branching', 'leaf_mass_ratio', 'sample_wt', 'leaf_sav', 'dmc')]) %>%  
  na.omit() 
```
...
#Group by flam. metric: 
PCA_metrics2 <- df.v %>% 
  filter(Variables %in% c("\nLFM", "Water Potential", "Sample Density",
           "\nLMA          ", "      Thickness\n", "Branching", "Leaf Ratio",
...
pca_df_main %>% 
  ...
  ggplot(aes(x = traits_pc_score, y = flamm_pc_score)) +
    ...
    #scale_color_manual(values = c("#00468BB2", "#42B540B2", "#0099B4B2", "#925E9FB2", "#FDAF91B2", "#AD002AB2", "#ADB6B6B2", "#1B1919B2")) +
    scale_color_lancet() + # if no ARTCAL, so manually define color palette
    theme_bw() +
...
```{r}
pc_labels <- c('PC1 (-FH)', 'PC2 (-\U0394Temp)', 'PC3 (FD)', 'PC1 (DMC, -Density, -LMR)', 'PC2 (Leaf SA:V, -Thick.)', 'PC3 (LMA, Sample Mass)')
