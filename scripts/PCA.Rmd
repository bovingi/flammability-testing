...
```{r}
flamm.df <- flamm.df %>% 
  filter(species %in% c('CEAGRI', 'ERIKAR', 'MALLAU', 'SALAPI', 'SALLEU', 'IRIDOU', 'MANDEN', 'HETARB')) %>% 
  filter(ignition != '0') %>% 
...
```{r}
pc_labels <- c('PC1 (Flamm.)', 'PC2 (Flamm.)', 'PC1 (Traits)', 'PC2 (Traits)', 'PC3 (Traits)')
names(pc_labels) <- c('PC1_flamm', 'PC2_flamm', 'PC1_traits', 'PC2_traits', 'PC3_traits')

pc.scatterplot <- pca_df_main %>% 
  pivot_longer(cols = c(PC1_flamm, PC2_flamm), values_to = 'flamm_pc_score', names_to = 'flamm_pc') %>% 
...
ggsave(here('figures', 'main_figures', 'pca.and.scatterplots.png'), height = 12, width = 10)
```
...
flamm.split.df <- flamm.df %>% 
  filter(species %in% c('CEAGRI', 'ERIKAR', 'MALLAU', 'SALAPI', 'SALLEU', 'HETARB', 'MANDEN')) # No IRIDOU
