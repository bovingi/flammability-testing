...
int <- colnames(predictors)
for(i in 1:length(int)){
  int2[i]<-paste0(int[i], ":", "species")
}
...
for(i in 2:ncol(predictors)){
    ...
    for(j in (1+length(call.vec)):(length(call)+length(call.vec))){ # adding linear mixed effects model to mod.list
      for(k in 1:length(int2)){
      mod.list[[j]] <- lmer(as.formula(paste('fh', '~', call[j-length(call.vec)], '+', int2[k], '+', '(1|plant_id)', sep = '')), data = mem.df)
      }
    }
