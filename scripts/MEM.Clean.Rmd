...
mem.df <- flamm.df %>% 
  ...
  select(species, thickness, lfm, leaf_area, LMA, sample_wt, leaf_mass_ratio, branching, mpa,
         ...
         ww_flam_sample, branch_volume, start_temp, leaf_area, plant_id) %>% 
  mutate(lfm = scale(lfm), mpa = scale(mpa), ww_flam_sample = scale(ww_flam_sample),
         dw_flam_sample = scale(dw_flam_sample), LMA = scale(LMA), thickness = scale(thickness),
         leaf_area = scale(leaf_area), branching = scale(branching),
         leaf_mass_ratio = scale(leaf_mass_ratio), sample_wt = scale(sample_wt),
...
mem.predictors.df <- mem.df %>% 
  select(thickness, lfm, LMA, sample_wt, leaf_mass_ratio, branching, mpa, dw_flam_sample, ww_flam_sample, start_temp, leaf_area, species) # twelve predictors
```
...
for (j in 1:length(BIC_vec)){
 ...
 glmmLasso(fh ~ species + mpa + ww_flam_sample + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
# 1e-05
fh_lambda <- lambda[which.min(AIC_vec)]
```

### Model Selection
```{r}
fh_lasso <- glmmLasso(fh ~ species + mpa + ww_flam_sample + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = fh_lambda,
...
LASSO does not help in this case... All variables are included

## Flame Duration
Because AIC/BIC based model selection came up with some solid results, we don't need to use LASSO
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(temp_change ~ species + mpa + sample_wt + ww_flam_sample + dw_flam_sample + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
lambda[which.min(AIC_vec)]
# 1e-05
tc_lambda <- lambda[which.min(AIC_vec)]
```

### Model Selection
```{r}
tc_lasso <- glmmLasso(temp_change ~ species + mpa + sample_wt + ww_flam_sample + dw_flam_sample + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
Again, LASSO is not helpful in this case, as none of the variables are selected
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(heat_flux_change ~ species + mpa + sample_wt + leaf_mass_ratio + ww_flam_sample,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
lambda[which.min(AIC_vec)]
# 1e-05
hf_lambda <- lambda[which.min(AIC_vec)]
```

### Model Selection
```{r}
hf_lasso <- glmmLasso(heat_flux_change ~ species + mpa + sample_wt + leaf_mass_ratio + ww_flam_sample,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
LASSO selects species and wet weight in the 'top model'; finally it helps us!

# REMEF Figures
## Flame Height
Note: from above models, selecting models with a variable we're interested in looking at in further detail

MPa:
```{r}
fh_mod <- lmer(fh ~ mpa + ww_flam_sample + species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_fh <- remef(fh_mod, fix = c('ww_flam_sample'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height
...
Wet Weight:
```{r}
mem.df$r_fh <- remef(fh_mod, fix = c('mpa'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of mpa and removing the random effects to isolate the relationship between wet weight and flame height

# With REMEF
ggplot(mem.df, aes(x = ww_flam_sample, y = r_fh))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 0.9)+ 
  stat_regline_equation(label.y = 1.2) +
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.fh.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = ww_flam_sample, y = r_fh))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 0.9)+ 
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.fh.spp.png'), width = 8, height = 5.5)
```

## Flame Duration
This one was a bit more clear, as the top model selected was branching + dry weight for both

Branching:
```{r}
fd_mod <- lmer(fd ~ branching + dw_flam_sample + species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_fd <- remef(fd_mod, fix = c('dw_flam_sample'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = branching, y = r_fd))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
...
Dry Weight:
```{r}
mem.df$r_fd <- remef(fd_mod, fix = c('branching'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = dw_flam_sample, y = r_fd))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 3.7)+ 
  stat_regline_equation(label.y = 4.2) +
  labs(x = 'Dry Weight (scaled)', y = 'Flame Duration (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
...
# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = dw_flam_sample, y = r_fd))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 0.9)+ 
...
Water Potential:
```{r}
hf_mod <- lmer(heat_flux_change ~ mpa + ww_flam_sample + species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_hf <- remef(hf_mod, fix = c('ww_flam_sample'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height
...
ggplot(mem.df, aes(x = mpa, y = r_hf))  +
  ...
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
  labs(x = 'Water Potential (scaled)', y = 'Heat Flux Change (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'mpa.vs.heatflux.png'), width = 8, height = 5.5)
...
ggplot(aes(x = mpa, y = r_hf))  +
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'mpa.vs.heatflux.spp.png'), width = 8, height = 5.5)
```
Wet weight:
```{r}
mem.df$r_hf <- remef(hf_mod, fix = c('mpa'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = ww_flam_sample, y = r_hf))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 3.8)+ 
  stat_regline_equation(label.y = 4.3) +
  labs(x = 'Wet Weight (scaled)', y = 'Heat Flux Change (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.heatflux.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = ww_flam_sample, y = r_hf))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 1.9)+ 
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.heatflux.spp.png'), width = 8, height = 5.5)
```


## Temp. Change
This one is a bit tricky, as the top model selected doesn't have anything we're particularly interested in, but AIC and BIC agree on the top model, so we should use it at least a bit

TOP MODEL
Dry weight:
```{r}
temp_mod <- lmer(temp_change ~ start_temp + dw_flam_sample + species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs
...
# With REMEF
ggplot(mem.df, aes(x = dw_flam_sample, y = r_temp))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 3.6)+ 
  stat_regline_equation(label.y = 4) +
  labs(x = 'Dry Weight (scaled)', y = 'Temp. Change (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'dw.vs.temp.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = dw_flam_sample, y = r_temp))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 1.9)+ 
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'dw.vs.temp.spp.png'), width = 8, height = 5.5)
```

Starting Temp.
```{r}
mem.df$r_temp <- remef(temp_mod, fix = c('dw_flam_sample'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height
...
MODEL 2: W/ WET WEIGHT
```{r}
temp_mod <- lmer(temp_change ~ start_temp + ww_flam_sample + species + (1|plant_id), data = mem.df, REML = F) # remef is also sensitive to NAs

mem.df$r_temp <- remef(temp_mod, fix = c('start_temp'), ran = "all", keep.intercept = FALSE) # w/ remef, we are removing the fixed effect of wet weight and removing the random effects to isolate the relationship between mpa and flame height

# With REMEF
ggplot(mem.df, aes(x = ww_flam_sample, y = r_temp))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
  labs(x = 'Wet Weight (scaled)', y = 'Temp. Change (effects removed, scaled)') +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.temp.png'), width = 8, height = 5.5)

# Split by Species
mem.df %>%  filter(species != 'HETARB') %>% 
ggplot(aes(x = ww_flam_sample, y = r_temp))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  facet_wrap(~species, scales = 'free') +
  stat_cor(label.y = 3.8)+ 
  ...
  theme(axis.title = element_text(face = 'bold', size = 15),
        axis.text = element_text(size = 12))
ggsave(here('figures', 'MEM', 'REMEF', 'ww.vs.temp.spp.png'), width = 8, height = 5.5)
```
