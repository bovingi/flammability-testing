...
trait.summary.plant_id <- flamm.df %>%  
  ...
  select(fh, species, lfm, leaf_lfm, stem_lfm, mpa, ww_flam_sample, dw_flam_sample, LMA, SLA, thickness, leaf_area, branching, stem_mass_ratio, leaf_mass_ratio, sample_wt, start_temp, branch_volume, sample_density, ambient_humidity, ambient_temp, thermocoupler_height, hotplate_height, plant_id) %>% 
...
flamm.df %>% 
  select(fh, fd, pfg, temp_change, heat_flux_change, species, lfm, leaf_lfm, stem_lfm, mpa, ww_flam_sample, dw_flam_sample, LMA, SLA, thickness, leaf_area, branching, stem_mass_ratio, leaf_mass_ratio, sample_wt, start_temp, branch_volume, sample_density, ambient_humidity, ambient_temp, thermocoupler_height, hotplate_height) %>% 
  ggcorr(label = T)
```
...
mem.df <- flamm.df %>% 
  ...
  select(species, thickness, lfm, leaf_area, LMA, sample_wt, leaf_mass_ratio, branching, mpa,
         ...
         ww_flam_sample, sample_density, branch_volume, start_temp, leaf_area, plant_id) %>% 
  mutate(lfm = scale(lfm), mpa = scale(mpa), ww_flam_sample = scale(ww_flam_sample),
...
mem.predictors.df <- mem.df %>% 
  select(thickness, lfm, LMA, sample_wt, leaf_mass_ratio, branching, mpa, sample_density, start_temp, leaf_area, species) # eleven predictors (note: no ww or dw)
```
...
for (j in 1:length(BIC_vec)){
 ...
 glmmLasso(fh ~ species + mpa + lfm + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fh_lasso <- glmmLasso(fh ~ species + mpa + lfm + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
AIC-based: branching and speciesMANDEN are both removed from consideration
BIC-based: branching, lfm, speciesMANDEN, speciesMALLAU are all removed from consideration
significance: sample weight and speciesHETARB (BIC-based only) are selected as significant predictors
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(fd ~ species + lfm + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
fd_lasso <- glmmLasso(fd ~ species + mpa + lfm + branching + sample_wt,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
AIC-based: lfm and speciesMALLAU are both removed from consideration
BIC-based: all variables removed from consideration
significance: sample weight and speciesERIKAR and SALAPI are selected as significant predictors (for AIC)
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(temp_change ~ species + mpa + branching + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
tc_lasso <- glmmLasso(temp_change ~ species + mpa + branching + sample_wt + start_temp,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = tc_lambda,
...
Variables Removed: mpa, branching, sample weight (!!), speciesMALLAU, IRIDOU, MANDEN
Significance: starting temp. and speciesHETARB both sig.
...
for (j in 1:length(BIC_vec)){
 ...
 glm1 <- try( #throwing main variables identified by the information criteria model selection into the LASSO cross-validation process; using plant.id as random effect
 glmmLasso(heat_flux_change ~ species + mpa + lfm + sample_wt + leaf_mass_ratio,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
 lambda = lambda[j],
...
### Model Selection
```{r}
hf_lasso <- glmmLasso(heat_flux_change ~ species + mpa + lfm + sample_wt + leaf_mass_ratio,
 data=mem.df,
 rnd = list(plant_id=~1), 
 family = gaussian(link = "identity"),
...
AIC: removes mpa, speciesIRIDOU
BIC: removes mpa, sample_wt (!!), speciesHETARB, IRIDOU, MANDEN, SALLEU
Significance: speciesMALLAU is significant, sample_wt is significant (only for AIC-based)
...
# With REMEF
ggplot(mem.df, aes(x = leaf_mass_ratio, y = r_hf))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
...
# With REMEF
ggplot(mem.df, aes(x = branching, y = r_temp))  +
  geom_point(alpha = 0.8, color = 'gray20') +
  geom_smooth(method = 'lm', color = 'gray40') +
  stat_cor(label.y = 1.9)+ 
  stat_regline_equation(label.y = 2.3) +
