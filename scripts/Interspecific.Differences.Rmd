...
```{r}
ww.spp <- lmer(ww_flam_sample ~ species + (1|plant_id), data = flamm.df)
anova(ww.spp)
ww.spp.tukey <- emmeans(ww.spp, pairwise ~ species, adjust = 'tukey')
ww.spp.tukey.df <- summary(ww.spp.tukey$contrasts)
ww.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'SALAPI', 'CEAGRI', 'SALLEU', 'MANDEN', 'IRIDOU', 'ARTCAL', 'ERIKAR'), letters = c('a', 'a', 'ab', 'bcd', 'cd', 'cd', 'd', 'd', 'd')) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
flamm.df %>%
  ...
  ggplot() +
    ...
    geom_text(aes(x = species, y = 20, label = letters), fontface = 'bold', data = ww.spp.tukey.labels) +
...
flamm.df %>%
  ...
  ggplot() +
    ...
    geom_text(aes(x = species, y = 0.55, label = letters), fontface = 'bold', data = mpa.spp.tukey.labels) +
