...
fh.spp.tukey.labels <- data.frame(species = c('HETARB', 'CEAGRI', 'SALAPI', 'MALLAU', 'ARCDEN', 'SALLEU', 'ARTCAL', 'IRIDOU', 'ERIKAR'), letters = c('a', 'ab', 'ab', 'ab', 'abc', 'bc', 'bc', 'c', 'c'), adj = c(0, 0, 4, 4, 4, 0, 0, 4, 0)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)

fh.boxplot <- flamm.df %>%
  mutate(species = fct_relevel(species, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU','ARCDEN', 'ARTCAL', 'IRIDOU', 'ERIKAR'))) %>% 
  ggplot() +
    ...
    scale_color_manual(values = c('#925E9FB2', '#AD002AB2', '#42B540B2', '#ADB6B6B2', '#1B1919B2','#00468BB2', '#ED0000B2', '#FDAF91B2', '#0099B4B2')) +
    theme_bw() +
...
fd.spp.tukey.labels <- data.frame(species = c('CEAGRI', 'MALLAU', 'HETARB', 'SALLEU', 'ARCDEN', 'SALAPI', 'IRIDOU', 'ARTCAL', 'ERIKAR'), letters = c('a', 'a', 'ab', 'abc', 'abcd', 'bcd', 'cd', 'cd', 'd'), adj = c(0, 4, 0, 0, 4, 4, 4, 0, 0)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)

fd.boxplot <- flamm.df %>%
  filter(fd < 70) %>% 
  mutate(species = fct_relevel(species, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU',
                                                 'ARCDEN', 'ARTCAL', 'IRIDOU', 'ERIKAR'))) %>% 
  ggplot() +
...
tc.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'SALLEU', 'SALAPI', 'CEAGRI', 'ARCDEN', 'ARTCAL', 'IRIDOU', 'ERIKAR'), letters = c('a', 'ab', 'ab', 'ab', 'ab', 'ab', 'b', 'b', 'b'),
...
hf.spp.tukey.labels <- data.frame(species = c('HETARB', 'SALAPI', 'MALLAU', 'CEAGRI', 'SALLEU', 'ARCDEN', 'IRIDOU', 'ARTCAL', 'ERIKAR'), letters = c('a', 'ab', 'abc', 'abc', 'bc', 'bc', 'bc', 'bc', 'c'), adj = c(0, 0.015, 0.015, 0, 0, 0.015, 0.015, 0, 0)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
sw.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'SALAPI', 'CEAGRI', 'SALLEU', 'ARCDEN', 'IRIDOU', 'ARTCAL', 'ERIKAR'), letters = c('a', 'a', 'b', 'bc', 'c', 'c', 'c', 'c', 'c'), 
...
mpa.spp.tukey.labels <- data.frame(species = c('CEAGRI', 'ARTCAL', 'ERIKAR', 'HETARB', 'MALLAU', 'SALLEU', 'ARCDEN', 'SALAPI', 'IRIDOU'), letters = c('a', 'b', 'bc', 'bc', 'bc', 'bcd', 'bcd', 'cd', 'd'), adj = c(0, 0, 0, 0, 0.025, 0, 0.025, 0.025, 0.025)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
branching.spp.tukey.labels <- data.frame(species = c('CEAGRI', 'ARCDEN', 'SALLEU', 'ARTCAL', 'MALLAU', 'HETARB', 'ERIKAR', 'SALAPI', 'IRIDOU'), letters = c('a', 'ab', 'ab', 'ab', 'bc', 'bc', 'c', 'c', 'c'),
...
```{r}
flamm.df %>%
  filter(species != 'ARCDEN') %>% 
  mutate(species = fct_relevel(species, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU', 'ARTCAL', 'IRIDOU', 'ERIKAR'))) %>% 
  ...
  scale_color_manual(values = c("#925E9FB2", "#AD002AB2", "#42B540B2", "#ADB6B6B2", '#1B1919B2', "#ED0000B2","#FDAF91B2", "#0099B4B2")) +
    theme_bw() +
    theme(legend.position = 'none', 
          axis.title = element_text(face = 'bold', size = 17),
          axis.text.x = element_text(size = 12, angle = 10, vjust = 0.9, hjust=0.75),
          axis.text.y = element_text(size = 15))
...
km_boxplot_flam <- comp_kmeans_df %>% 
  ...
  mutate(var = case_when(
    ...
    mutate(var = fct_relevel(var, levels = c('Flame Height', 'Flame Duration', 'Heat Flux Change',
                                                   'Temp. Change', 'Proportion Ignited', 'Live Fuel Moisture',
                                                   'Sample Weight', 'Dry Mass Content', 'Branching',
...
spp_vs_flamkm <- comp_kmeans_df %>% 
  mutate(species = fct_relevel(species.x, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU',
                                                 'ARCDEN', 'ARTCAL', 'IRIDOU', 'ERIKAR'))) %>% 
  ggplot(aes(x = species, y = cluster, color = as.factor(cluster))) +
    geom_jitter(width = 0.1, height = 0.1) +
...
ggsave(here('figures', 'main_figures', 'kmeans_cluster_arranged_figure.png'), height = 10, width = 16)
```
