...
### LFM
```{r}
lfm.spp <- lmer(lfm ~ species + (1|species_id), data = flamm.df)
anova(lfm.spp) # p < 0.00001
lfm.spp.tukey <- emmeans(lfm.spp, pairwise ~ species, adjust = 'tukey')
lfm.spp.tukey.df <- summary(lfm.spp.tukey$contrasts)
lfm.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU',
                                              'ARCDEN', 'ARTCAL', 'ERIKAR'),
                                  letters = c('b', 'b', 'ab', 'b', 'b', 'ab', 'a', 'ab'),
                                  adj = c(0, 15, 0, 15, 0, 15, 0, 15)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
lfm.boxplot <- flamm.df %>%
  ...
  ggplot() +
    ...
    geom_text(aes(x = species, y = 390 + adj, label = letters), fontface = 'bold',
              data = lfm.spp.tukey.labels, size = 6) +
    scale_color_manual(values = c('#925E9FB2', '#AD002AB2', '#42B540B2', '#ADB6B6B2', '#1B1919B2','#00468BB2', '#ED0000B2', '#0099B4B2')) +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 18),
...
### DMC
```{r}
dmc.spp <- lmer(dmc ~ species + (1|species_id), data = flamm.df)
anova(dmc.spp) # p < 0.00001
dmc.spp.tukey <- emmeans(dmc.spp, pairwise ~ species, adjust = 'tukey')
dmc.spp.tukey.df <- summary(dmc.spp.tukey$contrasts)
dmc.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU',
                                              'ARCDEN', 'ARTCAL', 'ERIKAR'),
                                  letters = c('b', 'b', 'ab', 'b', 'b', 'ab', 'a', 'ab'),
                                  adj = c(0, 1.5, 0, 1.5, 0, 1.5, 0, 1.5)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
dmc.boxplot <- flamm.df %>%
  ...
  ggplot() +
    ...
    geom_text(aes(x = species, y = 8 + adj, label = letters), fontface = 'bold',
              data = dmc.spp.tukey.labels, size = 6) +
    scale_color_manual(values = c('#925E9FB2', '#AD002AB2', '#42B540B2', '#ADB6B6B2', '#1B1919B2','#00468BB2', '#ED0000B2', '#0099B4B2')) +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 18),
...
### Leaf Mass Ratio
```{r}
lmr.spp <- lmer(leaf_mass_ratio ~ species + (1|species_id), data = flamm.df)
lmr.spp <- lm(leaf_mass_ratio ~ species, data = flamm.df)
lmr.aov <- aov(lmr.spp) # p < 0.00001
TukeyHSD(lmr.aov)
lmr.spp.tukey <- emmeans(lmr.spp, pairwise ~ species, adjust = 'tukey')
lmr.spp.tukey.df <- summary(lmr.spp.tukey$contrasts)
lmr.spp.tukey.labels <- data.frame(species = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU',
                                              'ARCDEN', 'ARTCAL', 'ERIKAR'),
                                  letters = c('b', 'b', 'ab', 'b', 'b', 'ab', 'a', 'ab'),
                                  adj = c(0, 0.15, 0, 0.15, 0, 0.15, 0, 0.15)) # manually extracted from above dataframe (couldn't figure out way to automate process w/ emmeans)
...
lmr.boxplot <- flamm.df %>%
  ...
  ggplot() +
    ...
    geom_text(aes(x = species, y = 0.56 + adj, label = letters), fontface = 'bold',
              data = lmr.spp.tukey.labels, size = 6) +
    scale_color_manual(values = c('#925E9FB2', '#AD002AB2', '#42B540B2', '#ADB6B6B2', '#1B1919B2','#00468BB2', '#ED0000B2', '#0099B4B2')) +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 18),
...
flamm.df %>%
  ...
  mutate(species = fct_relevel(species, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU', 'ARTCAL', 'ERIKAR'))) %>% 
  ...
  scale_color_manual(values = c("#925E9FB2", "#AD002AB2", "#42B540B2", "#ADB6B6B2", '#1B1919B2', "#ED0000B2", "#0099B4B2")) +
    theme_bw() +
    theme(legend.position = 'none', 
...
# expanded version
ggarrange(fh.boxplot, fd.boxplot, hf.boxplot, tc.boxplot, sw.boxplot, branching.boxplot, lfm.boxplot, dmc.boxplot, lmr.boxplot,
...
fh.boxplot <- interspec.df %>%
  mutate(species = fct_relevel(species, levels = c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU','ARCDEN', 'ARTCAL', 'ERIKAR'))) %>% 
  ggplot() +
