...
flam.df <- read.csv(here('data', 'Flamm_Data.csv'))
```
...
flam.df <- flam.df %>% 
  ...
  mutate(time_of_glow_end = as_hms(time_of_glow_end)) %>% 
...
heatflux.df.woop <- heatflux.df %>% 
  filter(date == flam.df$date[6]) %>%  #index row, select column for date
  filter(time >= flam.df$start_time[6] & time <= flam.df$time_of_glow_end[6])

loess.mod.CH7 <- loess(CH7 ~ as.numeric(time), data = heatflux.df.woop, span = 0.1)
...
loess.mod.CH8 <- loess(CH8 ~ as.numeric(time), data = heatflux.df.woop, span = 0.1)
