...
traits.df.long <- flamm.df %>% 
  dplyr::rename(sample_wt = total_branch_mass.y) %>% 
...
```{r}
## FD ## - sig.
fd.sw.lm <- lm(scale(fd) ~ scale(total_branch_mass.y), data = flamm.df)
summary(fd.sw.lm)
plot(fd.sw.lm)
ggplot(flamm.df, aes(x = total_branch_mass.y, y = fd)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(x = 'Sample Wt. (g)', y = 'Flame Duration (s)') +
  theme_bw()
## FH ## - sig.
fh.sw.lm <- lm(scale(fh) ~ scale(total_branch_mass.y), data = flamm.df)
summary(fh.sw.lm)
plot(fh.sw.lm)
ggplot(flamm.df, aes(x = total_branch_mass.y, y = fh)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(x = 'Sample Wt. (g)', y = 'Flame Height (cm)') +
  theme_bw()
## PFG ## - sig.
pfg.sw.lm <- lm(scale(pfg) ~ scale(total_branch_mass.y), data = flamm.df)
summary(pfg.sw.lm)
plot(pfg.sw.lm)
ggplot(flamm.df, aes(x = total_branch_mass.y, y = pfg)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(x = 'Sample Wt. (g)', y = 'Post-Flame Glow (s)') +
  theme_bw()
## LFM ## - sig.
lfm.sw.lm <- lm(lfm ~ total_branch_mass.y, data = flamm.df)
summary(lfm.sw.lm)
plot(lfm.sw.lm)
ggplot(flamm.df, aes(x = total_branch_mass.y, y = lfm)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(x = 'Sample Wt. (g)', y = 'Live Fuel Moisture (%)') +
  theme_bw()
## MPa ## - n.s.
mpa.sw.lm <- lm(mpa ~ total_branch_mass.y, data = flamm.df)
summary(mpa.sw.lm)
plot(mpa.sw.lm)
ggplot(flamm.df, aes(x = total_branch_mass.y, y = mpa)) +
  geom_point() +
  geom_smooth(method = 'lm') +
...
flamm.df.temp.fh <- flamm.df %>% 
  drop_na(fh, total_branch_mass.y) %>% 
  select(fh, total_branch_mass.y) %>% 
  mutate(fh = scale(fh), total_branch_mass.y = scale(total_branch_mass.y))
fh.sw.gam <- gam(fh ~ s(total_branch_mass.y), data = flamm.df.temp.fh)
summary(fh.sw.gam) 
flamm.df.temp.fh$predict.gam.fh <- predict.Gam(fh.sw.gam)
ggplot(data = flamm.df.temp.fh) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.fh), color = 'black') +
  geom_point(aes(x = total_branch_mass.y, y = fh), alpha = 0.5, color = 'gray50')

# FD
flamm.df.temp.fd <- flamm.df %>% 
  drop_na(fd, total_branch_mass.y) %>% 
  select(fd, total_branch_mass.y) %>% 
  mutate(fd = scale(fd), total_branch_mass.y = scale(total_branch_mass.y))
fd.sw.gam <- gam(fd ~ s(total_branch_mass.y), data = flamm.df.temp.fd)
summary(fd.sw.gam)
flamm.df.temp.fd$predict.gam.fd <- predict.Gam(fd.sw.gam)
ggplot(data = flamm.df.temp.fd) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.fd), color = 'black') +
  geom_point(aes(x = total_branch_mass.y, y = fd), alpha = 0.5, color = 'gray50')

# PFG
flamm.df.temp.pfg <- flamm.df %>% 
  drop_na(pfg, total_branch_mass.y) %>% 
  select(pfg, total_branch_mass.y) %>% 
  mutate(pfg = scale(pfg), total_branch_mass.y = scale(total_branch_mass.y))
pfg.sw.gam <- gam(pfg ~ s(total_branch_mass.y), data = flamm.df.temp.pfg)
summary(pfg.sw.gam)
flamm.df.temp.pfg$predict.gam.pfg <- predict.Gam(pfg.sw.gam)
ggplot(data = flamm.df.temp.pfg) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.pfg), color = 'black') +
  geom_point(aes(x = total_branch_mass.y, y = pfg), alpha = 0.5, color = 'gray50')
```

Generalized Additive Models 2
```{r}
# FH
fh.sw.gam <- gam(fh ~ lo(total_branch_mass.y), data = flamm.df.temp.fh)
summary(fh.sw.gam)
flamm.df.temp.fh$predict.gam.fh <- predict.Gam(fh.sw.gam)
ggplot(data = flamm.df.temp.fh) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.fh), color = 'black') +
...
# FD
fd.sw.gam <- gam(fd ~ lo(total_branch_mass.y), data = flamm.df.temp.fd)
summary(fd.sw.gam)
flamm.df.temp.fd$predict.gam.fd <- predict.Gam(fd.sw.gam)
ggplot(data = flamm.df.temp.fd) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.fd), color = 'black') +
...
# PFG
pfg.sw.gam <- gam(pfg ~ lo(total_branch_mass.y), data = flamm.df.temp.pfg)
summary(pfg.sw.gam)
flamm.df.temp.pfg$predict.gam.pfg <- predict.Gam(pfg.sw.gam)
ggplot(data = flamm.df.temp.pfg) +
  geom_point(aes(x = total_branch_mass.y, y = predict.gam.pfg), color = 'black') +
...
for(i in 1:5){
fh.sw.poly <- lm(data = flamm.df.temp.fh, fh ~ poly(total_branch_mass.y, i))
...
for(i in 1:5){
fd.sw.poly <- lm(data = flamm.df.temp.fd, fd ~ poly(total_branch_mass.y, i))
...
for(i in 1:5){
pfg.sw.poly <- lm(data = flamm.df.temp.pfg, pfg ~ poly(total_branch_mass.y, i))
...
# Running models
fh.sw.poly <- lm(data = flamm.df.temp.fh, fh ~ poly(total_branch_mass.y, 3))
summary(fh.sw.poly)
fd.sw.poly <- lm(data = flamm.df.temp.fd, fd ~ poly(total_branch_mass.y, 3))
summary(fd.sw.poly)
pfg.sw.poly <- lm(data = flamm.df.temp.pfg, pfg ~ poly(total_branch_mass.y, 3))
summary(pfg.sw.poly)
```
...
```{r}
# Data wrangling
quantile(flamm.df$total_branch_mass.y, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)

flamm.df <- flamm.df %>% 
  mutate(sample_wt_bins = case_when(
    total_branch_mass.y <= 1.1064 ~ '0.2264-1.1064',
    total_branch_mass.y > 1.1064 & total_branch_mass.y <= 2.8064  ~ '1.1064-2.8064',
    total_branch_mass.y > 2.8064 & total_branch_mass.y <= 8.4864 ~ '2.8064-8.4864',
    total_branch_mass.y > 8.4864 & total_branch_mass.y <= 30.5664 ~ '8.4864-30.5664',
  ))
...
flamm.df.sw <- flamm.df %>% 
  mutate(fh = scales::rescale(fh),
         ...
         total_branch_mass.y = scales::rescale(total_branch_mass.y)) %>% 
  mutate(fh.norm = fh/total_branch_mass.y,
         fd.norm = fd/total_branch_mass.y,
         pfg.norm = pfg/total_branch_mass.y) %>% 
  mutate(fh.norm.sqrt = fh/(1+total_branch_mass.y^1/2),
         fd.norm.sqrt = fd/(1+total_branch_mass.y^1/2),
         pfg.norm.sqrt = pfg/(1+total_branch_mass.y^1/2)) %>% 
  mutate(fh.norm.cube.root = fh/(1+total_branch_mass.y^1/3),
         fd.norm.cube.root = fd/(1+total_branch_mass.y^1/3),
         pfg.norm.cube.root = pfg/(1+total_branch_mass.y^1/3))

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fh)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fh.norm)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fh.norm.sqrt)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fh.norm.cube.root)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fd)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fd.norm)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fd.norm.sqrt)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = fd.norm.cube.root)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = pfg)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = pfg.norm)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = pfg.norm.sqrt)) +
    geom_point()

flamm.df.sw %>% 
  ggplot(aes(x = total_branch_mass.y, y = pfg.norm.cube.root)) +
    geom_point()
