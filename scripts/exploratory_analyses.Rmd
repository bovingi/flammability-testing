...
flamm.df.long <- flamm.df %>% 
  select(date, species, plant, rep, mpa, branching, branch_height, branch_length, branch_width, leaf_lfm, leaf_lfm, stem_lfm, lfm, LMA, SLA, thickness, sample_wt, fh, fd, tti, pfg, max_temp, temp_change, max_heat_flux_loessCH7, max_heat_flux_loessCH8, dw_flam_sample, ww_flam_sample, branch_volume, heat_flux_change) %>%  # took out  gd, gti, ttfg for now
  pivot_longer(!c(date, species, plant, rep, mpa, branching, branch_height, branch_length, branch_width, leaf_lfm, stem_lfm, lfm, LMA, SLA, thickness, sample_wt, dw_flam_sample, ww_flam_sample, branch_volume), names_to = 'trait', values_to = 'value') %>% 
...
flamm_pca_df <- flamm.df %>% 
  select(fd, fh, prop_ig, heat_flux_change, temp_change, pfg, species) # removed tti because that's mostly tied to timing of manual ignition
...
traits_pca_df <- flamm.df %>% 
  select(leaf_mass_ratio, leaf_lfm, stem_lfm, lfm, thickness, mpa, SLA, dw_flam_sample, ww_flam_sample, branch_volume, species)
...
flamm_kmeans_scaled_df <- flamm_kmeans_df %>% 
  select(-c(species_id, species, id, prop_ig)) %>%  #removing prop_ig for now as it's directly tied to species
  scale() %>% 
  as.data.frame()
...
fviz_nbclust(flamm_kmeans_scaled_df, kmeans, method = "wss") #k = 3 or k = 7
# 2. silhouette method
...
flamm_kmeans_df %>% 
  ...
  mutate(kmeans.flam.index = (`3` + `1`)/(`2` + `1` + `3`)) %>% 
...
flamm.df.manual.long <- flamm.df.manual %>% 
  select(date, species, plant, rep, fh, fd, tti, pfg, max_temp, max_heat_flux_loessCH7, max_heat_flux_loessCH8) %>%  # took out  gd, gti, ttfg for now
...
flamm.df %>% 
  ...
  ggplot(aes(x = auc_voc, y = val)) +
    geom_point() +
    geom_smooth(se = F, method = 'lm', color = 'gray30') +
    facet_wrap(~var, scales = 'free') + 
    labs(x = 'VOCs (Area Under Curve)', y = 'Flammability Metric Value') +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 17),
...
flamm.df %>% 
  pivot_longer(cols = c(fh, fd, pfg, prop_ig, temp_change, heat_flux_change), names_to = 'var', values_to = 'val') %>% 
  ggplot(aes(x = log(avg_voc), y = val)) +
    geom_point() +
    geom_smooth(se = F, method = 'lm', color = 'gray30') +
    facet_wrap(~var, scales = 'free') + 
    labs(x = 'VOCs/second (log-transformed)', y = 'Flammability Metric Value') +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 17),
          axis.text.y = element_text(size = 15),
...
flamm.df %>% 
  pivot_longer(cols = c(fh, fd, pfg, prop_ig, temp_change, heat_flux_change), names_to = 'var', values_to = 'val') %>% 
  ggplot(aes(x = log(norm_auc_voc), y = val)) +
    geom_point() +
    geom_smooth(se = F, method = 'lm', color = 'gray30') +
    facet_wrap(~var, scales = 'free') + 
    labs(x = 'VOCs (AUC; Normalized, log-transformed)', y = 'Flammability Metric Value') +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 17),
          axis.text.y = element_text(size = 15),
...
flamm.df %>% 
  pivot_longer(cols = c(fh, fd, pfg, prop_ig, temp_change, heat_flux_change), names_to = 'var', values_to = 'val') %>% 
  ggplot(aes(x = log(norm_avg_voc), y = val)) +
    geom_point() +
    geom_smooth(se = F, method = 'lm', color = 'gray30') +
    facet_wrap(~var, scales = 'free') + 
    labs(x = 'VOCs/second (Normalized, log-transformed)', y = 'Flammability Metric Value') +
    theme_bw() +
    theme(legend.position = 'none',
          axis.title = element_text(face = 'bold', size = 17),
          axis.text.y = element_text(size = 15),
...
# Summary Plots: Sample Size vs. FD/FH
Per request by KF
```{r}
...
flam_labels <- c('Flame Height', 'Flame Duration')
names(flam_labels) <- c('fh', 'fd')
...
# By species
spp_vector <- c('SALAPI', 'SALLEU', 'CEAGRI', 'ARTCAL', 'MALLAU')
plot_list <- list()
