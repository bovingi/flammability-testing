...
flamm.df.long <- flamm.df %>% 
  select(date, species, plant, rep, mpa, total_branch_mass, fh, fd, tti, pfg, max_temp, max_heat_flux, max_heat_flux_loessCH7, max_heat_flux_loessCH8) %>%  # took out  gd, gti, ttfg for now
...
flamm.df.long %>% 
  filter(trait %in% c('fd', 'fh', 'max_heat_flux', 'max_heat_flux_loessCH7', 'max_heat_flux_loessCH8', 'max_temp', 'pfg', 'tti')) %>% 
ggplot(aes(x = as.numeric(total_branch_mass), y = value, color = species)) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_smooth(method = 'lm', linewidth = 1, alpha = 0.8, se = F) +
...
ggplot(flamm.df.prop.ig, aes(x = species, y = prop.ig, color = species)) +
  scale_y_continuous(limits = c(0,1)) +
  geom_point(size = 6) +
  geom_text(aes(label = n, x = species, y = prop.ig + 0.1)) +
