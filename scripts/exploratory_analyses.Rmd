...
traits_pca_df <- flamm.df %>% 
  select(leaf_mass_ratio, leaf_lfm, stem_lfm, lfm, thickness, mpa, SLA, dw_flam_sample, ww_flam_sample, branch_volume, species, ignition)
...
# DELETE LATER - JUST TO SHOW KF SOMETHING
```{r}
flamm_pca_df %>% 
  select(-prop_ig) %>% 
  pivot_longer(cols = !c(species, ignition), values_to = 'val', names_to = 'var') %>% 
  ggplot(aes(x = species, y = val, color = ignition)) +
    geom_boxplot() +
    theme_bw() +
    labs(x = 'Species', y = 'Flam. Metric Value', color = 'Ignition') +
